  ...
  <div class="detail_container">
    <v-container class="main_wrapper">
      ...
      <v-list elevation="1" subheader>
        ...
        <v-combobox
          @change="
            $store.state.NomenclaturesModule.entry.id_family = $event
              ? $event.id
              : null
          "
          v-model="$store.state.NomenclaturesModule.entry._family"
          :disabled="
            $store.state.NomenclaturesModule.loadingList ||
            !$store.getters.stateEditCreate($route.query.action)
          "
          :items="$store.state.NomenclaturesModule.listFamily"
          :loading="$store.state.NomenclaturesModule.loadingList"
          chips
...
export default {
  ...
  async mounted() {
    await this.$store.dispatch(
      "NomenclaturesModule/getListEntries",
  ...
  watch: {
    "$store.state.NomenclaturesModule.entry.id": {
      handler(newValue) {
        ...
        this.$store.dispatch(
          "setTitle",
          this.$store.state.NomenclaturesModule.entry
        );
      },
    },
    "$route.query.action": {
