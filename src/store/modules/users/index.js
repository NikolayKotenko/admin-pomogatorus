...
export default {
    ...
    actions: {
        ...
        async deleteEntry({commit, dispatch}) {
            ...
            await Request.delete(this.state.BASE_URL+'/users/delete-user/'+this.state.UsersModule.entry.id)
        ...
        async onSubmit({commit, dispatch}) {
            if (this.state.UsersModule.entry.email == null)
                return false;
        ...
        async deleteSetGroupUser(_, idUser){
            ...
            for (const item of this.state.UsersModule.entry.groups) {
                ...
                const response = await Request.post(this.state.BASE_URL+'/m-to-m/users-groups/',newEntry)
        ...
        async createEntry({commit}, objRequest){
            try {
                return await Request.post(
                    this.state.BASE_URL+'/users/set-user-data', objRequest)
            } catch (e) {
                console.log(e)
        ...
        async getListEntries({commit}, id) {
            ...
            try {
                const result = await Request.get(this.state.BASE_URL+'/users/get-list-users')
        ...
        async getListGroups({commit}){
            const response = await Request.get(this.state.BASE_URL+'/entity/groups');
